...
#
/sbin/modprobe sbs_battery

IMG_PATH="/usr/share/factory/images"
BATTERY_PATH="/sys/class/power_supply/sbs-20-000b"
# If you change MIN_LEVEL, be sure to change the "charging.png" entry in
# images.yaml, and regenerate charging.png.
MIN_LEVEL=50
TTY="/dev/tty1"
...
if [[ $(get_percentage) -le $MIN_LEVEL ]]; then
  # Ask for AC power
  if (ectool powerinfo | grep Device | grep -v -q 20010$); then
    ply-image --clear 0x000000 "${IMG_PATH}/connect_ac.png"
    while (ectool powerinfo | grep Device | grep -v -q 20010$); do
      sleep 0.5;
    done
  fi
